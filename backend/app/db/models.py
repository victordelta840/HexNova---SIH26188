...
class DocumentReferenceProfile(Base):
    __tablename__ = "document_reference_profiles"
    ...
    profile_summary: Mapped[dict | None] = mapped_column(JSON, default=dict)
    characteristics: Mapped[dict | None] = mapped_column(JSON, default=dict)
    created_by: Mapped[str | None] = mapped_column(String(128))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    locked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    last_action: Mapped[str | None] = mapped_column(String(64))

    __table_args__ = (
        Index("ix_reference_profiles_document_type", "document_type"),
        Index("ix_reference_profiles_status", "status"),
        Index("ix_reference_profiles_created_at", "created_at"),
    )
